...
void Handle_str(const Element *elem, char *output[], const int *row, const int len)
{
...
void Handle_img(const Element *elem, char *output[], int *row)
{
    ...
    const char *temp = elem->nature.nature_content.src_content;
    while (count--)
    {
...
{
    if (elem->nature.nature_content.nature_content[direction] == row)
    { // 方向：row
        if (elem->nature.nature_content.nature_content[align_items] == start)
        { // 竖直：start  ->  right.
            ...
            else if (elem->nature.nature_content.nature_content[justify_content] == center || elem->nature.nature_content.nature_content[justify_content] == space_evenly)
            { // 水平：center
                int r = *rows, count = 0;
                while (count < elem->content.child.children_number)
                {
                    if (elem->content.child.children[count]->name == h || elem->content.child.children[count]->name == p)
                    {
                        ...
                        Handle_str(elem->content.child.children[count], output, &r, elem->content.child.children[count]->size.width[1]);
                        output[r] += elem->size.width[1] - (elem->size.width[1] - elem->content.child.children[count]->size.width[1]) / 2;
                        r += 1;
                        *rows = r;
                    }
                    else if (elem->content.child.children[count]->name == img)
                    {
                        ...
                        while (*rows < r)
                        {
                            output[*rows] += elem->size.width[1] - (elem->size.width[1] - elem->content.child.children[count]->size.width[1]) / 2;
                            *rows += 1;
                        }
                    }
                    else if (elem->content.child.children[count]->name == dive)
        ...
        else if (elem->nature.nature_content.nature_content[align_items] == end)
        { // 竖直：end
            int height = Blank_height(elem);
            while (height--)
            {
        ...
        else if (elem->nature.nature_content.nature_content[align_items] == space_evenly)
        { // 竖直：evenly
            int height = Blank_height(elem);
    ...
    else if (elem->nature.nature_content.nature_content[direction] == column)
    { // 方向：column
        if (elem->nature.nature_content.nature_content[align_items] == start)
        { // 竖直：start
            if (elem->nature.nature_content.nature_content[justify_content] == start)
        ...
        else if (elem->nature.nature_content.nature_content[align_items] == center || elem->nature.nature_content.nature_content[align_items] == space_evenly)
        { // 竖直：center||evenly
            if (elem->nature.nature_content.nature_content[justify_content] == start)
            { // 水平：start
                int width = Blank_width(elem);
                ...
                while (count < elem->content.child.children_number)
                {
                    ...
                    int temp4 = elem->size.height[1] - elem->content.child.children[count]->size.height[1] - (elem->size.height[1] - elem->content.child.children[count]->size.height[1]) / 2;
                    while (temp4--)
                    {
                        output[r++] += elem->content.child.children[count]->size.width[1];
                    }
            ...
            else if (elem->nature.nature_content.nature_content[justify_content] == end)
            {
                ...
                *h1 = elem->size.height[1];
                ...
                while (count < elem->content.child.children_number)
                {
                    int r = rsv, temp3 = (elem->size.height[1] - elem->content.child.children[count]->size.height[1]) / 2;
                    while (temp3--)
                    {
                        output[r++] += elem->content.child.children[count]->size.width[1];
                    }
